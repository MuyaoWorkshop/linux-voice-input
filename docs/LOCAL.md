# 本地 Whisper 方案 - 完整指南

基于 OpenAI Whisper 的离线语音转文字工具，完全本地运行，隐私保护。

## 目录

- [方案特点](#方案特点)
- [系统要求](#系统要求)
- [快速安装](#快速安装)
- [使用方法](#使用方法)
- [守护进程模式](#守护进程模式)
- [故障排查](#故障排查)
- [性能优化](#性能优化)

---

## 方案特点

### 核心优势

- ✅ **完全离线** - 无需网络连接，数据不上传
- ✅ **隐私保护** - 所有处理在本地完成
- ✅ **双模式运行** - 普通模式 / 守护进程模式可切换
- ✅ **极速启动** - 守护进程模式 <0.5 秒响应
- ✅ **实时反馈** - 守护进程模式支持音量条和倒计时
- ✅ **免费开源** - 无API费用，完全免费

### 运行模式对比

| 模式 | 启动速度 | 内存占用 | 适用场景 |
|------|---------|---------|---------|
| **普通模式** | 4-5 秒 | 0MB 常驻 | 偶尔使用（日均 1-2 次） |
| **守护进程模式** | <0.5 秒 🚀 | ~900MB 常驻 | 频繁使用（日均 10+ 次） |

---

## 系统要求

### 硬件要求
- **CPU**: 任意 x86_64 处理器
- **内存**: 最低 4GB，推荐 8GB+
- **磁盘**: ~500MB (模型 + 依赖)
- **麦克风**: 可用的音频输入设备

### 软件要求
- **系统**: Debian 12 / Ubuntu 20.04+ / Arch / Fedora
- **桌面**: GNOME (推荐) / KDE / XFCE / i3wm
- **Python**: 3.8+

### 性能说明

**本方案基于 CPU 运行**（无需 GPU），典型性能：
- **模型**: Whisper base
- **首次加载**: ~3 秒
- **识别 5 秒语音**: ~3-5 秒
- **CPU 占用**: 50-70%（4核CPU）

---

## 快速安装

### 使用安装脚本（推荐）

项目提供了统一的安装脚本，支持自动配置：

```bash
cd <项目目录>
./install.sh
```

**安装脚本会自动完成：**
1. 检查系统依赖（Python、xclip、portaudio 等）
2. 创建项目虚拟环境（venv/）
3. 安装 Python 依赖包
4. 下载 Whisper base 模型（如果未存在）
5. 可选：配置守护进程服务
6. 可选：配置 GNOME 快捷键

**选项说明：**
- 选择方案时选 `1) 仅安装本地 Whisper` 或 `3) 安装双方案`
- 守护进程模式可选（推荐频繁使用者启用）
- 快捷键自动配置为 Super+V

### 手动安装

如果你想手动控制每一步：

#### 1. 安装系统依赖

```bash
sudo apt update
sudo apt install -y \
    portaudio19-dev \
    python3-pyaudio \
    xclip \
    pulseaudio-utils \
    ffmpeg
```

#### 2. 创建虚拟环境

```bash
cd <项目目录>
python3 -m venv venv
source venv/bin/activate
```

#### 3. 安装 Python 依赖

**方法一：正确安装（推荐）**

先安装 CPU 版本的 PyTorch：

```bash
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu
pip install openai-whisper pyaudio
```

**方法二：直接安装**

```bash
pip install openai-whisper pyaudio
```

> 如果看到 NVIDIA 相关警告，不用担心，我们使用 CPU 模式。

#### 4. 下载模型

首次运行会自动下载，也可以手动预下载：

```bash
python3 -c "import whisper; whisper.load_model('base')"
```

模型会下载到 `~/.cache/whisper/base.pt`（约 140MB）

#### 5. 设置权限

```bash
chmod +x local/voice_input.py
chmod +x local/voice_input_wrapper.sh
chmod +x local/voice_input_fast.sh
chmod +x local/switch_mode.sh
```

#### 6. 配置快捷键（可选）

**GNOME：**

```bash
gsettings set org.gnome.settings-daemon.plugins.media-keys custom-keybindings \
"['/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/voice-input/']"

gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:\
/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/voice-input/ \
name "语音输入"

gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:\
/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/voice-input/ \
command "<项目目录>/local/voice_input_wrapper.sh"

gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:\
/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/voice-input/ \
binding '<Super>v'
```

**其他桌面环境：**
- KDE Plasma: 系统设置 → 快捷键 → 自定义快捷键
- XFCE: 设置 → 键盘 → 应用程序快捷键
- i3wm: 编辑 `~/.config/i3/config`，添加：
  ```
  bindsym $mod+v exec <项目目录>/local/voice_input_wrapper.sh
  ```

---

## 使用方法

### 命令行测试

```bash
cd <项目目录>
source venv/bin/activate
./local/voice_input.py
```

**预期输出：**
```
正在加载 Whisper 模型...
模型加载完成: base

🎤 开始录音... (说话后停顿2秒自动结束)
...........
检测到静音，停止录音
录音结束
正在识别...
识别结果: 今天天气很好

✓ 已复制到剪贴板，可使用 Ctrl+V 粘贴
```

### 快捷键使用（推荐）

**普通模式：**
1. 按 `Super + V`
2. 等待 4-5 秒加载模型
3. 对着麦克风说话（10秒内）
4. 停顿 2 秒自动结束
5. 识别完成，自动复制
6. 按 `Ctrl + V` 粘贴

**守护进程模式：**
1. 按 `Super + V`
2. <0.5 秒立即开始录音 🚀
3. 实时显示音量条
4. 说话，停顿 2 秒自动结束
5. 显示静音倒计时
6. 识别完成，自动复制
7. 按 `Ctrl + V` 粘贴

---

## 守护进程模式

### 什么是守护进程模式？

守护进程模式通过在后台常驻运行，预加载 Whisper 模型，实现极速启动：

- **启动速度**: 从 4-5 秒降至 <0.5 秒
- **响应速度**: 提升 **87.5%**
- **用户体验**: 实时音量条 + 静音倒计时
- **资源占用**: ~900MB 内存常驻，CPU 空闲时仅 1.6%

### 启用守护进程模式

#### 方法一：使用安装脚本

运行 `./install.sh` 时选择启用守护进程模式。

#### 方法二：手动切换

```bash
cd <项目目录>/local
./switch_mode.sh daemon
```

**输出示例：**
```
=== 切换到守护进程模式 ===

1. 启动守护进程...
   ✓ 守护进程已启动
2. 更新快捷键配置...
   ✓ 快捷键已更新为守护进程模式

=== 切换完成 ===

守护进程模式特点：
  ✓ 启动速度极快 (<0.5秒)
  ✓ 实时音量条显示
  ⚠ 常驻内存 (~900MB)

现在按 Super+V 试试，应该能立即开始录音！
```

### 模式切换

```bash
# 查看当前模式
./switch_mode.sh status

# 切换到守护进程模式（快速）
./switch_mode.sh daemon

# 切换到普通模式（节省内存）
./switch_mode.sh normal

# 自动切换（守护进程 ↔ 普通）
./switch_mode.sh toggle
```

### 管理守护进程

```bash
# 启动
systemctl --user start voice-input-daemon

# 停止
systemctl --user stop voice-input-daemon

# 重启
systemctl --user restart voice-input-daemon

# 查看状态
systemctl --user status voice-input-daemon

# 查看日志
journalctl --user -u voice-input-daemon -f

# 开机自启动
systemctl --user enable voice-input-daemon

# 禁用开机自启动
systemctl --user disable voice-input-daemon
```

### 资源占用详情

**守护进程模式（空闲状态）：**

| 资源 | 占用 | 说明 |
|------|------|------|
| **内存** | ~900MB | Whisper base 模型 + Python 运行时 |
| **CPU** | ~1.6% | 使用 select() 优化，几乎可忽略 |
| **磁盘 I/O** | 0 | 模型常驻内存，无磁盘读写 |
| **网络** | 0 | 完全本地，无网络通信 |

**工作状态：**
- CPU：录音时 ~20-30%，识别时 ~150%（多核）
- 内存：峰值 ~1GB

---

## 故障排查

### 问题 1: 无法录音

**错误信息**:
```
ALSA lib pcm.c:2722:(snd_pcm_open_noupdate) Unknown PCM
```

**解决方法**:
```bash
# 检查麦克风设备
arecord -l

# 测试录音
arecord -d 5 test.wav
aplay test.wav

# 检查 PulseAudio
pulseaudio --check
pulseaudio --start
```

### 问题 2: 未找到 xclip

**错误信息**:
```
❌ 错误: 未找到 xclip 命令
```

**解决方法**:
```bash
sudo apt install xclip
```

### 问题 3: PyAudio 编译失败

**错误信息**:
```
fatal error: portaudio.h: No such file or directory
```

**解决方法**:
```bash
sudo apt install portaudio19-dev python3-pyaudio
pip install pyaudio
```

### 问题 4: 识别不准确

**可能原因和解决方法**:

1. **环境噪音太大**
   - 在安静环境使用
   - 使用质量更好的麦克风

2. **说话不清晰**
   - 说话速度适中
   - 吐字清晰

3. **模型太小**
   - 编辑脚本，修改模型：
     ```python
     model = whisper.load_model("small")  # 或 "medium"
     ```
   - 更大的模型更准确，但更慢

### 问题 5: 识别速度太慢

**优化方法**:

1. **使用更小的模型**
   ```python
   model = whisper.load_model("tiny")  # 最快
   ```

2. **关闭其他占用 CPU 的程序**

3. **性能对比**:
   | 模型 | 准确率 | 速度 (4核CPU) |
   |------|--------|--------------|
   | tiny | ~80% | 2-3秒 |
   | base | ~85% | 4-5秒 |
   | small | ~90% | 8-12秒 |
   | medium | ~95% | 20-30秒 |

### 问题 6: ModuleNotFoundError

**错误信息**:
```
ModuleNotFoundError: No module named 'whisper'
```

**解决方法**:
```bash
# 激活虚拟环境
cd <项目目录>
source venv/bin/activate

# 重新安装依赖
pip install openai-whisper pyaudio
```

### 问题 7: 快捷键不工作

**检查步骤**:

1. 验证脚本可执行
   ```bash
   ls -l <项目目录>/local/voice_input_wrapper.sh
   # 应该有 x 权限
   ```

2. 手动运行包装脚本
   ```bash
   <项目目录>/local/voice_input_wrapper.sh
   ```

3. 检查快捷键配置
   - 打开 **设置** → **键盘** → **查看和自定义快捷键**
   - 查找 "语音输入"
   - 确认快捷键和命令路径正确

### 问题 8: 守护进程无法启动

**检查方法**:
```bash
# 查看状态
systemctl --user status voice-input-daemon

# 查看日志
journalctl --user -u voice-input-daemon -n 50

# 检查服务文件
cat ~/.config/systemd/user/voice-input-daemon.service

# 重新加载配置
systemctl --user daemon-reload
systemctl --user restart voice-input-daemon
```

---

## 性能优化

### Whisper 模型选择

| 模型    | 大小  | 内存  | 速度 (CPU) | 准确率 | 推荐场景          |
|---------|-------|-------|------------|--------|-------------------|
| tiny    | 75MB  | ~1GB  | 快 (~2s)   | ~75%   | 快速笔记          |
| base    | 150MB | ~2GB  | 中 (~4s)   | ~85%   | 日常使用（推荐）  |
| small   | 490MB | ~3GB  | 慢 (~10s)  | ~90%   | 重要文档          |
| medium  | 1.5GB | ~5GB  | 很慢 (~30s)| ~95%   | 专业场景          |
| large   | 3GB   | ~10GB | 极慢 (~60s)| ~98%   | 不推荐（CPU 太慢）|

### 性能测试数据

典型性能参考（4核Intel i5处理器）：

| 语音长度 | tiny   | base   | small  |
|----------|--------|--------|--------|
| 3 秒     | 1.5s   | 2.8s   | 7.2s   |
| 5 秒     | 2.1s   | 4.3s   | 10.5s  |
| 10 秒    | 3.5s   | 7.8s   | 18.2s  |

### 录音参数调整

编辑 `local/voice_input.py` 调整参数：

```python
# 录音时长（秒）
RECORD_SECONDS = 10

# 静音检测阈值
SILENCE_THRESHOLD = 500

# 采样率
SAMPLE_RATE = 16000
```

---

## 卸载

如果需要卸载本地方案：

```bash
# 1. 停止守护进程（如果启用）
systemctl --user stop voice-input-daemon
systemctl --user disable voice-input-daemon

# 2. 删除服务文件
rm -f ~/.config/systemd/user/voice-input-daemon.service
systemctl --user daemon-reload

# 3. 删除虚拟环境和代码
cd <项目目录>
rm -rf venv/
rm -rf local/

# 4. 删除模型缓存（可选）
rm -rf ~/.cache/whisper/

# 5. 删除快捷键（GNOME）
# 在设置 → 键盘中手动删除 "语音输入"
```

---

## 常见问题

### Q: 支持英文识别吗？

**A**: 支持，修改脚本指定语言：
```python
result = model.transcribe("audio.wav", language="en")
```

### Q: 识别结果没有标点符号？

**A**: 正常现象，Whisper 离线模型不支持自动添加标点。如需标点，请使用讯飞云方案。

### Q: 只能识别10秒吗？

**A**: 当前设计如此，避免内存占用过高。如需长文本，请使用讯飞云方案（无时长限制）。

### Q: 守护进程模式值得吗？

**A**: 如果你：
- ✅ 每天使用 10+ 次 → 每月节省 45 分钟
- ✅ 内存充足（16GB+）→ 900MB 占用可忽略
- ✅ 追求体验 → 即按即用，思路不被打断

那么**非常值得**！

### Q: 如何确认守护进程正常工作？

**A**: 按 `Super+V` 后：
1. 如果立即（<0.5秒）打开终端 → 守护进程正常
2. 如果等待 4-5 秒 → 守护进程未运行，执行 `./switch_mode.sh daemon`

---

## 支持

- **项目地址**: https://github.com/MuyaoWorkshop/linux-voice-input
- **问题反馈**: https://github.com/MuyaoWorkshop/linux-voice-input/issues
- **常见问题**: [FAQ.md](FAQ.md)
